...
| 0.3.4 | `ez split` — turn a multi-commit branch into a stack with one branch per commit, then `ez submit` for a PR each. Rewrites nothing: the commits already form a chain, so each layer is a branch ref pointed at a commit already in place plus a recorded parent (`cmd/split.rs`, pure `plan_split`). The split branch keeps its name, tip, and any existing PR, becoming the top layer; new branches are `<branch>-1`, `<branch>-2`, ... below it (`--prefix` overrides). Refuses before creating anything on a name collision or a merge commit in the range; no-ops on a single-commit branch; `--dry-run` previews. `ez move` now restructures the GitHub native stack it moves within instead of reporting a success it did not achieve: it detects the PR's native stack *before* any local mutation, skips the `update_pr_base` call GitHub always rejects for a stacked PR, and dissolves/recreates the stack via `repair_native_stack_exact`. Move receipts split local from remote — `local`, `restacked`, `pr_base` (`updated` \| `deferred_to_native_stack` \| failed-with-reason \| null), and `native_stack_before`. Sync's reconciliation moves to the shared `cmd/native_stack::reconcile_stacks` so both commands restructure identically. |
